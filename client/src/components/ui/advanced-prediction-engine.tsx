  ...
  const createAdvancedPrediction = useMutation({
    mutationFn: async (data: any) => {
      ...
      return {
        ...
        predictedIrr: Math.round(predictedIrr),
        successProbability: Math.round(successProbability),
        riskLevel: predictedIrr >= 16 ? "low" : predictedIrr >= 12 ? "medium" : "high",
        confidenceScore: Math.round(Math.min(98, Math.max(85, 94 * locationMultiplier * stabilityMultiplier))),
        financialProjection: {
          initialInvestment,
          annualRevenue,
        ...
        riskAssessment: {
          technicalRisk: Math.round(Math.max(5, 25 - (locationConfig?.gridStability || 7) * 2)),
          financialRisk: Math.round(Math.max(8, 30 - predictedIrr)),
          regulatoryRisk: data.location.includes("Ghana") ? 12 : 18,
          environmentalRisk: projectConfig?.value === "hydro" ? 22 : 15,
          marketRisk: Math.round(Math.max(10, 35 - (projectConfig?.marketGrowth || 15)))
        },
        esgImpact: {
